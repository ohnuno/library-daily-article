...
| 変数名 | 内容 |
|--------|------|
| `GEMINI_API_KEY` | Google AI StudioのAPIキー（プロジェクト名：tufs-daily-article） |
...
# 東京外国語大学 ROR ID（変更不要。他機関導入時は差し替え）
TUFS_ROR_ID=https://ror.org/03f4q9q08
```
...
③ ISSNフィルタをJSTORに切り替えて再検索
  ...
  ↓ 0件
④ 教員著作論文フォールバック
   a. CrossRef × ROR ID（TUFS_ROR_ID）で検索
