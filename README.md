...
| `TUFS_ROR_ID` | 教員著作論文フォールバック用の機関 ROR ID（変更任意） |

ROR ID は [ror.org](https://ror.org) で機関名を検索して確認してください。
東京外国語大学の ROR ID：`https://ror.org/03f4q9q08`
...
**他機関で導入する場合の必須変更**：
`.github/workflows/daily.yml` の `TUFS_ROR_ID` を自機関の ROR ID に変更してください。
...
Copyright (c) 2026 ohnuno / Tokyo University of Foreign Studies Library
...
> このプロジェクトは東京外国語大学図書館が開発・公開するオープンソースソフトウェアです。
